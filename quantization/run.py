...
def parse_args():
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Path to FP32 .pt state_dict (preferred for quantization)",
    )
    p.add_argument(
        "--weights-script",
        type=str,
        default=None,
        help="Path to FP32 TorchScript .script (used only for baseline size/latency)",
    )
    ...
    p.add_argument(
        "--calib-steps", type=int, default=50, help="PTQ: number of calibration batches"
    )
    p.add_argument(
        "--qat-steps",
        type=int,
        default=200,
        help="QAT: number of short training steps (very small)",
    )
    ...
    p.add_argument(
        "--bench-cpu",
        action="store_true",
        help="Run a tiny CPU latency benchmark FP32 vs INT8",
    )
...
def main():
    ...
    if HAS_DATA:
        try:
            ...
            print(
                f"[WARN] get_loaders failed ({e}); falling back to synthetic calibration."
            )
    ...
    fp32_model = create_model(
        args.model, args.encoder, args.in_ch, NUM_CLASSES, pretrained=False
    ).cpu()
    ...
    print(
        f"[SIZE] FP32 TorchScript: {fp32_script_path.name} -> {fp32_size / 1e6:.2f} MB"
    )

    if args.weights_script and Path(args.weights_script).is_file():
        ws = Path(args.weights_script)
        print(
            f"[INFO] Provided FP32 script for reference: {ws} ({ws.stat().st_size / 1e6:.2f} MB)"
        )

    if args.method == "ptq":
        print("\n================= [PTQ] =================")
        int8_model = ptq_fx_convert(
            fp32_model, example, train_loader, calib_steps=args.calib_steps
        )
    else:
        print("\n================= [QAT] =================")
        int8_model = qat_fx_convert_short_train(
            fp32_model, example, train_loader, val_loader, steps=args.qat_steps
        )
    ...
    print(
        f"[DELTA] Size change: {(fp32_size - q_size) / 1e6:.2f} MB (positive = smaller after quant)"
    )
    ...
    if args.bench_cpu:
        ...
        print(
            f"[LATENCY CPU] FP32: {fp32_lat * 1000:.2f} ms | INT8: {int8_lat * 1000:.2f} ms | speedup x{fp32_lat / max(int8_lat, 1e-6):.2f}"
        )
