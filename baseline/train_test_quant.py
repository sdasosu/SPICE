...
class SegLightningModuleTimed(pl.LightningModule):
    def __init__(
        ...
        self.register_buffer(
            "train_conf", torch.zeros(num_classes, num_classes, dtype=torch.long)
        )
        self.register_buffer(
            "val_conf", torch.zeros(num_classes, num_classes, dtype=torch.long)
        )
    ...
    def forward(self, x):
        ...
        if isinstance(out, dict):
            ...
            if out is None:
                raise RuntimeError(
                    "Model forward returned dict without 'out'/'logits'."
                )
        return out

    def configure_optimizers(self):
        if self.external_scheduler is None:
    ...
    def training_step(self, batch, batch_idx):
        images, target = (
            ...
            else (batch["image"], batch["mask"])
        )
        logits = self.forward(images)
        loss = self.criterian(logits, target)
        with torch.no_grad():
            preds = logits.argmax(dim=1)
            _update_confusion_matrix(self.train_conf, preds, target, self.num_classes)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self.criterian(logits, target)
        preds = logits.argmax(dim=1)
        _update_confusion_matrix(self.val_conf, preds, target, self.num_classes)
    ...
    def on_validation_epoch_end(self):
        ...
        print(
            f"[Epoch {self.current_epoch}] VAL  mIoU={miou:.4f} | AvgAcc={avg_acc:.4f}"
        )
        print(f"  per-class IoU: {np.array2string(iou, precision=4)}")
        print(f"  per-class Acc: {np.array2string(acc, precision=4)}\n")
...
def train_validation_timed(
    ...
    lit_module = SegLightningModuleTimed(
        model,
        criterian,
        num_classes,
        model_name,
    ...
    ckpt_cb = ModelCheckpoint(
        dirpath=str(save_root / "checkpoints" / "fullsize" / model_name),
        filename=f"{model_name}_lightning",
        monitor="val_mIoU",
        mode="max",
        save_top_k=1,
    ...
    print(f"\n=== [TRAINER END] tag={tag} ===\n")
    return {
        "best_miou": float(lit_module.best_val_miou),
...
@torch.no_grad()
def evaluate_one_epoch_val(
    model: nn.Module, val_loader: DataLoader, device: torch.device, num_classes: int = 5
):
    ...
    for batch in val_loader:
        images, target = (
            (batch[0].to(device), batch[1].to(device).long())
            if isinstance(batch, (list, tuple))
